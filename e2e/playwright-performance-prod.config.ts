...
const config = {
  ...
  testIgnore: '*.contract.perf.spec.ts', //Run everything except contract tests which require marks in dev mode
  timeout: 60 * 1000,
  ...
  projects: [
    {
      name: 'chrome-memory',
      testMatch: '*.memory.perf.spec.ts', //Only run memory tests
      use: {
    ...
    {
      name: 'chrome',
      testIgnore: '*.memory.perf.spec.ts', //Do not run memory tests without proper flags
      use: {
        browserName: 'chromium'
      }
    }
