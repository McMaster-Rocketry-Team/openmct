...
const config = {
  ...
  testMatch: '*.contract.perf.spec.ts', //Run everything except contract tests which require marks in dev mode
  timeout: 60 * 1000,
  ...
  projects: [
    {
      name: 'chrome',
      testIgnore: '*.memory.perf.spec.ts', //Do not run memory tests without proper flags. Shouldn't get here
      use: {
        browserName: 'chromium'
      }
    }
